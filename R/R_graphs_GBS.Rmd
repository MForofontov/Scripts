...
```{r}
ena661k <- read.table(file = "GBS/ena661k_data.csv", 
                   sep = ";", header=TRUE)
refseq <- read.table(file = "GBS/refseq_data.csv", 
                   sep = ";", header=TRUE)
labstrains <- read.table(file = "GBS/labstrains_data_updated.csv", 
                   sep = ";", header=TRUE)
```
...
columns_names <- c("ST","ena661k","refseq","labstrains")
...
m_unique_values_sum <- m_unique_values_sum[m_unique_values_sum[,2] >= 100,]
...
ggplot(data=m_unique_values_filtered, aes(x=value, y=ST, fill = variable)) +
  ...
  theme(plot.title = element_text(hjust = 0.5)) +  geom_text(data = totals ,aes(x=total,y=ST,label=total,fill=NULL),nudge_x = 100, size = 3)
```
```{r}
...
columns_names <- c("serotype","ena661k","refseq","labstrains")
...
ggplot(data=m_unique_values_filtered, aes(x= value, y=serotype, fill = variable)) + labs(y = "M types") +
  ...
  theme(plot.title = element_text(hjust = 0.5)) + geom_text(data = totals ,aes(x=total,y=serotype,label=total,fill=NULL),nudge_x= 100, size = 3)
...
```{r}
Count_dataset <- data.frame(Dataset = c("ena661k","Refseq","Labstrains"), 
                            Size = c(nrow(ena661k),nrow(refseq),nrow(labstrains)))
```
...
ggplot(Count_dataset, aes(x="", y=Size, fill=Dataset)) +
  ...
  coord_polar("y", start=0) + scale_fill_manual(values = c("#440154FF","#3B528BFF","#21908CFF","#5DC863FF","#FDE725FF"), 
                                                labels =   c("ena661k","Refseq","Labstrains")) + 
