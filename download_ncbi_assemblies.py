...
def main(input_table, output_directory, file_extension,
        ...
        for index, line in enumerate(lines[1:]):
            if line[15].strip() != '':
                ...
                with urllib.request.urlopen(line[15] + "/assembly_status.txt") as status:
                    ...
                    if str.split(status)[0] == "status=latest":
                    ...
                    # refseq + genebank
                        ...
                        with urllib.request.urlopen(line[14] + "/assembly_status.txt") as status:
                            ...
                            if str.split(status)[0] == "status=latest":
                                ...
                                status_dict[line[14].split('/')[-1]] = "Both suppressed"
                                lines.remove(line)
                    else:
                        lines.remove(line)
    ...
    with open(os.path.join(output_directory,"status.txt"),"w") as status_out:
        for name, value  in status_dict.items(): 
            status_out.write('{}:{}'.format(name,value))
    status_out.close()
